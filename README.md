...
2. Any methods in the Glue code path annotated with Cucumber's @Before is run before any Cucumber steps are run for the current Cucumber Scenario.  The walkthrough below applies to awtf_core, but you can see how this can be updated for your needs by looking at MyAppHooks in awtf-app-example.
    1. CoreHooks is the only class that has a Cucumber @Before annotation.  Hooks classes also need to be given a ContextConfiguration so Spring can create the created beans from your configuration.
    ```java
    ...
    ```
    2. CoreConfig is run from CoreHooks.  After it complements the following things in CoreConfig are now available for the test run:
        1. A TestEnvironmentConfig
        2. An AppConfig that can be used to looking up element locators specific to your app.
        3. A TestInstance which consists of a TestEnvironmentConfig and an AppConfig and has methods to interact with the Selenium Web Driver instance.
        4. A CoreStepsUtil which are helper methods used by the Cucumber steps.
        5. A CoreStepHandler that handles the Cucumber core steps.
...
5. Whether a scenario passes or fails the @After annotated methods in the Glue code are run.  There is only one @After method in the core framework which is the teardown method in CoreHooks.java.  Refer to MyAppHooks in awtf-app-example to update this as needed for your app.
6. Steps 1-4 repeat until there are no Cucumber Scenarios to run by the Test Runner.

NOTE: Again you can setup your Test Runners to use your own Hooks and ignore the ones in the core framework.  This shown in awtf-app-example project.
